...
def parseargs():
    ...
    htxt = {
        ...
        "--list-duplicates": "Find and list all duplicates in all or specified categories. If you want to find duplicates in specific categories, use (--destination, -D).",
        "--list-categories": "List all categories. If you want to list specific categories, use (--destination, -D).",
...
""",
        ...
        "--quiet":        "Be quiet. Don't print warnings. Errors will still be printed.",
    ...
    group.add_option("-N", "--no-newline",  action = "store_const", const = ', ', default = '\n', dest = "nonewline", help = htxt["--no-newline"])
    ...
    group.add_option("-d", "--delete",                          dest = "delete",    help = htxt["--delete"],    metavar = "CNR")
    group.add_option("-m", "--move",    type="int",             dest = "move",      help = htxt["--move"],      metavar = "CNR")
    group.add_option("-c", "--change",                          dest = "change",    help = htxt["--change"],    metavar = "CNR")
    ...
    group.add_option("-i", "--info",                            dest = "info",      help = htxt["--info"],  metavar = "CNR")
    group.add_option("-l", "--list",                            dest = "list",      help = htxt["--list"],  metavar = "CNR")
    group.add_option("-g", "--list-categories", action = "store_true", default = False, dest = "listcats",  help = htxt["--list-categories"])
    group.add_option("-u", "--list-duplicates", action = "store_true", default = False, dest = "listdupe",  help = htxt["--list-duplicates"])
    ...
    group.add_option("-P", "--profile", action = "store_const", default = "default", dest = "profile", help = htxt["--profile"], metavar = "NAME")
    group.add_option("-F", "--force",   action = "store_true",  dest = "force", help = htxt["--force"])
    group.add_option("-p", "--pretend", action = "store_true", default = False, dest = "pretend", help = htxt["--pretend"])
    group.add_option("-y", "--days-ago", type = "int", default = 0, dest = "daysago", help = htxt["--days-ago"], metavar = "DAYS")
    parser.add_option_group(group)
